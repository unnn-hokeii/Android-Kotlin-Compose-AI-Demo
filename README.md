...
4. 验证环境：终端执行 `adb --version`，输出版本信息即配置成功。

### 3\.2 国内镜像配置（解决网络超时问题）

由于国内网络环境限制，需配置阿里云镜像解决Gradle下载超时、依赖无法拉取的问题，核心配置如下：

1. 项目根目录 `settings.gradle.kts` 配置，替换原有仓库配置：
...
2. Gradle发行版镜像配置：修改 `gradle/wrapper/gradle-wrapper.properties`，将`distributionUrl`替换为国内腾讯云镜像：
...
模块级 `app/build.gradle.kts` 核心配置，确保Compose版本兼容、功能完整：
...
1. 打开Android Studio，点击`New Project`，选择`Empty Compose Activity`模板，点击`Next`。
...
3. 点击`Finish`完成项目创建，等待Gradle同步完成。

#### 1\.2 基础项目验证

1. 打开 `app/src/main/java/com/example/myfirstkotlinapp/MainActivity.kt`，查看自动生成的基础Compose代码。

2. 创建Android模拟器：选择ARM64架构的系统镜像（适配M2芯片），完成模拟器创建。

3. 点击运行按钮，项目成功编译并在模拟器中运行，显示默认`Hello Android!`界面，任务1完成。

#### 1\.3 截图要求

- 项目创建完成后的Android Studio项目结构界面

- 模拟器运行默认Hello World界面截图
...
掌握Jetpack Compose核心布局组件的用法，实现可交互的可展开列表布局，包含`Column`垂直布局、`Card`容器、`Text`文本展示、点击展开/收起的状态管理。

#### 2\.2 代码实现

替换`MainActivity.kt`全量代码：
...
2. 运行到模拟器，界面展示双条目卡片，点击`Show more`展开文本，点击`Show less`收起文本，交互正常。
...
1. 代码同步无报错，预览面板正常展示AI Demo界面。

2. 模拟器运行后，顶部栏、相机预览区、结果区、按钮区四大模块完整展示。

3. 交互测试：

    - 拍照识别：填充模拟识别结果、置信度、推理时间

    - 切换模型：在MobileNet与ResNet50之间切换
...
### 5\.2 README\.md 参考内容（项目根目录）

```Markdown
# Android-Kotlin-Compose-AI-Demo
实验2：构建Kotlin应用并使用Compose布局

## 实验介绍
本项目基于Kotlin + Jetpack Compose 开发Android应用，分为三大模块：基础Compose布局练习、AI演示界面开发，适配M2芯片MacBook Pro环境。

## 运行环境
- 设备：M2芯片 MacBook Pro
- 开发工具：Android Studio 2024.1+
- 编译版本：JDK 11
- 核心框架：Jetpack Compose 稳定版
- 最低支持版本：API 24 (Android 7.0)

## 功能说明
1. 基础布局Demo：Column、Card、状态管理，实现卡片展开/收起交互
2. AI演示界面：顶部导航栏、相机预览区、识别结果展示区、功能按钮区
3. 交互功能：拍照识别、切换模型、相册导入、清空结果

## 部署说明
1. 克隆项目
`git clone https://github.com/hokeii-rr/Android-Kotlin-Compose-AI-Demo.git`
2. 使用Android Studio打开项目
3. 配置国内阿里云、腾讯云镜像，同步Gradle
4. 运行模拟器（ARM64架构）启动项目

## 镜像配置
项目已内置阿里云Maven镜像、腾讯云Gradle镜像，解决国内下载超时问题。
```
...
|序号|问题描述|根本原因|解决方案|
|---|---|---|---|
|1|Gradle下载超时，提示`java.net.SocketTimeoutException`|国内网络无法访问Gradle官方仓库|将Gradle发行版地址替换为腾讯云国内镜像，配置阿里云Maven仓库加速依赖下载|
|2|同步失败，提示`Failed to resolve androidx`相关依赖|依赖包无法从国外仓库拉取|在`settings.gradle.kts`中配置阿里云镜像，优先从国内仓库拉取依赖|
|3|代码爆红，提示`Unresolved reference 'Arrangement'`|导入路径错误，Arrangement属于layout子包|修正导入语句：`androidx.compose.foundation.layout.Arrangement`|
|4|Card组件报错，提示无`backgroundColor`参数|Material3不同版本参数命名不兼容|使用标准写法：`colors = CardDefaults.cardColors(containerColor = xxx)`|
|5|TopAppBar颜色配置出现警告|低版本Material3接口兼容问题|使用全版本通用 `topAppBarColors` 方法配置样式|

## 七、实验总结

本次实验完成了基于Kotlin与Jetpack Compose的Android应用全流程开发，熟练掌握了Android Studio环境搭建、国内镜像配置、Git代码托管等基础操作。同时深入学习了Compose声明式UI的核心思想，掌握`Column`、`Row`、`Box`、`Card`等主流布局组件，以及Compose状态管理机制。

通过AI演示界面的开发，完成了综合布局组合、组件嵌套、交互逻辑编写，理解了Compose相比传统XML布局代码简洁、逻辑统一、实时预览的优势。实验过程中排查并解决了网络、依赖、API兼容等常见问题，提升了Android Compose项目的排错与实战能力。
